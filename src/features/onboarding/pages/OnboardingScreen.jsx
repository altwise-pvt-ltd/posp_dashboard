...
const STEP_SAVED_ALERTS = {
  pan: {
    ...
    message: "Your PAN information was captured.",
  },
  email: {
  ...
  aadhaar: {
    title: "Aadhaar saved",
    message: "Your Aadhaar details were captured.",
  },
  selfie: {
    title: "Selfie added",
    message: "Your selfie was captured successfully.",
  },
  bank: {
    title: "Bank account added",
    message: "Your bank details were saved.",
  },
  education: {
    title: "Education saved",
    message: "Your education details were captured.",
  },
  business: {
    title: "Business saved",
    message: "Your business details were captured.",
  },
};
...
function OnboardingNotice({ children }) {
  return (
    <FunnelLayout header="auto">
      <div
        className={`${FUNNEL_SHELL} flex min-h-[60vh] items-center justify-center py-4 sm:py-6 lg:py-8`}
      >
        {children}
      </div>
    </FunnelLayout>
...
export default function OnboardingScreen() {
  ...
  const handleSubmit = async () => {
    ...
    showAlert({
      ...
      message: [
        result.message ??
          "Your details are with our team — we'll verify them shortly.",
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 0:
        return <PanStep onNext={saveAndNext("pan")} />;
      case 1:
      ...
      case 4:
        return <BankStep onNext={saveAndNext("bank")} />;
      case 5:
        return <EducationStep onNext={saveAndNext("education")} />;
      case 6:
        return (
          <BusinessStep onNext={saveAndNext("business")} onSkip={skipAndNext} />
        );
      case 7:
        return <ReviewStep onSubmit={handleSubmit} />;
      default:
        return <StepPlaceholder step={STEPS[currentStep]} onNext={goNext} />;
    }
  };
  ...
  return (
    <FunnelLayout header="auto">
      ...
      <div className={`${FUNNEL_SHELL} py-4 sm:py-6 lg:py-8`}>
        {/* Progress stepper */}
        <div className="anim-fade mb-4 lg:mb-5 w-full">
          <Stepper steps={stepperSteps} activeIndex={currentStep} />
        </div>

        {/* Form + Sidebar — single column on mobile, two columns from lg up.
              Every width in the wizard lives here. The step cards set none of
              their own: they are `w-full` and fill whatever they are given, so
              this file is the only place a form width is decided.

              Both tracks are pinned rather than content-sized, so the grid
              measures the same on every step and the illustration beside it
              ...
              Below lg there are no tracks, so the form wrapper carries the
              phone/tablet caps instead — same job, stated once.
              ...
              few pixels instead of overflowing the shell. Otherwise it sits at
              its full 24rem / 26rem and never moves.

              Review spans the whole width instead — no sidebar there. */}
        <div
          className={
            "mx-auto w-full grid grid-cols-1 gap-10 items-start justify-center " +
            (isReview
              ? "max-w-4xl xl:max-w-7xl"
              : "lg:grid-cols-[22.75rem_minmax(0,24rem)] xl:grid-cols-[22.75rem_minmax(0,26rem)] lg:max-w-max")
          }
        >
          <div
            className={
              "anim-fade-d2 min-w-0 w-full" +
              (isReview ? "" : " max-w-89 sm:max-w-91 lg:max-w-none mx-auto")
            }
          >
            {renderStep()}
          </div>
