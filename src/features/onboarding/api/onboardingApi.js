...
/**
 * Document fetches in flight or already done, keyed by document key.
 *
 * The review screen asks for the same documents several times over. Both of its
 * layouts are mounted at once — only CSS hides one — so every document is
 * requested by a mobile summary card *and* by a desktop documents row, and the
 * selfie is requested a third time as the identity rail's avatar. Uncached,
 * this application's seven documents cost fifteen authenticated requests per
 * visit, and another fifteen after each inline edit refetches the review.
 *
 * Caching by key is safe indefinitely: a key names one immutable upload
 * (`onboarding/<app>/pan/front/<guid>.jpg`), so replacing a document yields a
 * new key rather than new bytes behind the old one.
 *
 * The promise is cached rather than the Blob, so simultaneous callers — which
 ...
 * revoking the URL it makes — and must not revoke the Blob itself, since other
 * callers hold the same one.
 *
 * A failed fetch is evicted from the cache, so a document that 404s or times
 * out once is retried on the next render rather than being permanently broken.
 */
export function fetchDocumentBlob(key) {
  const cached = documentBlobRequests.get(key);
  if (cached) return cached;

  const request = api
    .get(ENDPOINTS.onboarding.getDocument(key), { responseType: 'blob' })
  ...
  documentBlobRequests.set(key, request);
  return request;
}
